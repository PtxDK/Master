...
from heartnet.data.base_loader import load3D
...
BATCH_SIZE = 16
EPOCHS = 500
net = UNet3D(N_CLASSES, depth=4, dim=128)
loss = SparseCategoricalCrossentropy(from_logits=True)
opt = Adam(5.0e-05, 0.9, 0.999, 1e-8, decay=0.0)
net.compile(opt, loss=loss, metrics=["accuracy", dice], run_eagerly=True)
train_ds = load3D("/homes/pmcd/Peter_Patrick3/train").batch(BATCH_SIZE)
val_ds = load3D("/homes/pmcd/Peter_Patrick3/val").batch(BATCH_SIZE)
cbs = [
